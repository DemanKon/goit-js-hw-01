...
function getElementWidth(content, padding, border) {
    const modContent = parseFloat(content);
    const modPadding = parseFloat(padding);
    const modBorder = parseFloat(border);
    const width = modContent + modPadding * 2 + modBorder * 2;
    return `Загальна ширина елемента: ${width}`
}
